...
import LaundryServiceCard from '../components/laundry/LaundryServiceCard.jsx';
...
export default function LaundryServices() {
  ...
  const { addLaundry, count } = useCart();
  ...
  const handleAdd = (service) => {
    addLaundry(service);
    notify(`${service.name} added — choose pickup & return in your cart`, 'success');
  };
  ...
  return (
    <main className="flex-1 min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 p-4 sm:p-6">
      ...
      <section className="mx-auto mb-6 max-w-7xl">
        <div className="overflow-hidden rounded-3xl bg-gradient-to-br from-sky-600 to-blue-700 px-6 py-8 text-white shadow-lg sm:px-10 sm:py-10">
          <h1 className="text-2xl font-extrabold sm:text-3xl">Laundry Services</h1>
          <p className="mt-2 max-w-2xl text-sm text-sky-100 sm:text-base">
            Pick the services you need, then choose a <span className="font-semibold text-white">pickup</span> and a{' '}
            <span className="font-semibold text-white">return</span> time in your cart. Book a pickup at the same slot as a
            shop delivery and you’re only charged delivery once — we make a single trip.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto mb-8 max-w-7xl">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full sm:max-w-md">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search laundry services…" className="w-full rounded-xl border border-gray-200 bg-white py-3 pl-11 pr-4 text-sm text-gray-800 shadow-sm outline-none transition focus:border-sky-400 focus:ring-2 focus:ring-sky-100" />
          </div>
      ...
      <section className="mx-auto max-w-7xl">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center text-gray-400">
            ...
            <p className="mt-4 text-lg font-semibold text-gray-500">No laundry services found</p>
          ...
          <>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filtered.map((s, i) => (
                <LaundryServiceCard key={s._id} service={s} index={i} mounted={mounted} onAdd={handleAdd} canBook={!isAdmin} />
              ))}
            </div>
          </>
        )}
