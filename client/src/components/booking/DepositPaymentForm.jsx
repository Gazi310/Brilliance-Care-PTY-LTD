...
/**
 * The deposit payment form — MOCK provider.
 *
 * This component owns the "pay with card" block so that when real Stripe
 * lands, only this file changes (swap the inputs for Stripe's
 * <PaymentElement /> and call stripe.confirmPayment instead of onPay).
...
export default function DepositPaymentForm({ amount, onPay, busy }) {
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-line bg-white p-4 shadow-soft sm:p-5">
      ...
      >
        {busy ? 'Processing…' : `Pay $${amount.toFixed(2)} deposit`}
      </button>
