...
export default function LaundryServiceCard({ service, index = 0, mounted = true, onAdd, canBook = true }) {
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute -inset-1 rounded-[26px] bg-gradient-to-br from-sky-500 to-blue-600 opacity-0 blur-lg transition-opacity duration-500 group-hover:opacity-40" />
      ...
      <div className="relative flex h-full flex-col overflow-hidden rounded-3xl border border-white/70 bg-white shadow-md transition-all duration-500 ease-out group-hover:-translate-y-1.5 group-hover:shadow-2xl">
        <div className="relative flex h-40 items-center justify-center overflow-hidden bg-gradient-to-br from-sky-50 to-blue-50">
          {photo ? (
          ...
          {service.turnaround && (
            <span className="absolute right-3 top-3 z-20 rounded-full bg-white/90 px-2.5 py-1 text-[11px] font-bold text-sky-700 shadow-sm">
              ⏱ {service.turnaround}
            </span>
          )}
        </div>
        ...
        <div className="flex flex-1 flex-col p-5">
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            {canBook && (
              <button
                onClick={handleAdd}
                className="relative inline-flex items-center gap-1.5 overflow-hidden rounded-xl bg-gradient-to-r from-sky-500 to-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md transition-all duration-300 hover:shadow-lg active:scale-95"
              >
